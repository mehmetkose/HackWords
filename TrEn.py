...
class term_prog:
    ...
    def interface(self):
        ...
        while runner is True:
            ...
            waddstr(window_1, 'HackWords v0.3 by Fydor Dostoyevski.', attr=A_REVERSE)
...
def main():
    ...
    if OtherTools().check_connectivity('http://tureng.com') is False:
        ...
        sys.exit(1)
    words = get_words_from_net().get_items(file_name)
    os.chdir('Dict/')
    word_counter = 0
    ...
    for word in words:
        ...
        if OtherTools().check_not_found(word) is True:
            ...
            continue
        try:
            format_new_file().get_lines_between('%s.uf' % word, word)
        except TypeError:
            continue
        os.system("rm %s.uf" % word)
